...
## 10. 🥊 라라에듀 vs 휴넷 — 13축 분류 (Battle Card)

`competitor_research/hunet-deep-dive.md`의 13축 비교를 한 화면에 정리. 어디서 이기고 어디서 따라가야 하는지 즉답.

```mermaid
mindmap
  root((라라 vs 휴넷<br/>13축 비교))
    ★ 라라 우위 차별화 강화
      컴플라이언스 깊이<br/>11구간 단가 + 20% 동시할인
      산업안전 사고 사례<br/>공단 데이터 + 252사 익명 통계
    휴넷 우위 7개 벤치마킹
      카테고리 깊이 9 × 27 × 30+
      셀프 견적 카탈로그
      환급금 표 컬럼 즉시 노출
...
> 우위 2 / 열위 7 / 동등 4. 열위 7개 중에서도 **B1 카테고리 트리 + B2 환급금 노출**이 가장 빠른 효과 영역.

---

## 11. 🎯 벤치마킹 + 차별화 우선순위 — 임팩트 × 구현 난이도

휴넷 비교에서 도출한 12개 액션(B1-B7, D1-D5)을 어디부터 손댈지 결정.

```mermaid
quadrantChart
    title 라라에듀 액션 우선순위 — 임팩트 × 구현 난이도
    x-axis 쉬움 --> 어려움
    ...
    quadrant-3 점진 개선
    quadrant-4 미루기
    "D1 회사별 자동 맞춤화": [0.6, 0.95]
    "D2 사고 사례 추천": [0.55, 0.92]
    "B7 멤버십 모델": [0.85, 0.88]
    "B1 카테고리 트리": [0.3, 0.85]
    "B2 환급금 자동 계산": [0.25, 0.85]
    "D4 PM 도구 풀스택": [0.75, 0.78]
    "D3 제조업 특화": [0.35, 0.72]
    "B3 셀프 견적 카탈로그": [0.7, 0.7]
    "D5 즉시 견적 자동": [0.2, 0.65]
    "B6 AI 콘텐츠 빠른 추가": [0.35, 0.55]
    "B5 마이크로/몰입 분리": [0.4, 0.4]
    "B4 북러닝 패턴": [0.55, 0.4]
```

> **즉시 손댈 4개 (좌상단 — 빠른 효과)**: B1 카테고리 트리, B2 환급금 노출, D5 즉시 견적, D2 사고 사례
> **큰 베팅 3개 (우상단 — Phase 2 핵심)**: D1 자동 맞춤화, B7 멤버십, D4 PM 도구
> **미루기 2개**: B4 북러닝, B5 마이크로/몰입
...
| **10. vs 휴넷 13축** | 우위 2 / 열위 7 / 동등 4 | 영업 차별화 메시지 정할 때 |
| **11. 액션 우선순위** | 12개 액션 임팩트 × 난이도 | 다음 스프린트 정할 때 |
