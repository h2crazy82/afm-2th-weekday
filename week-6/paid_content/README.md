...
## ✨ 프리미엄 — AI 맞춤 프롬프트 생성 (NEW)

5개 기본 템플릿으로 부족한 상황을 위한 추가 기능. `/custom` 라우트에서 회사·업종·요구사항을 입력하면 Claude API가 즉석에서 새 프롬프트를 생성.

- **가격**: ₩39,000 (1회 생성, 영구 보관)
- **모델**: claude-haiku-4-5
- **DB**: `custom_generations` 테이블 (RLS — 본인만 열람)
- **플로우**: 폼 → draft 주문 → 토스 결제 → 토스 승인 → Claude API → 결과 저장 → /custom/[id]
- **에러 처리**: AI 호출 실패 시에도 결제는 유지하되 row에 에러 기록, 관리자 문의 안내
- **환경변수**: `ANTHROPIC_API_KEY` 필요. 없으면 /custom 진입 시 결제 차단 + "AI 모듈 셋업 중" 안내만 노출 (앱 자체는 정상 동작)
